...
def sjc_3d(
    ...
    **kwargs,
    ...
    # load nerf view
    images_, _, poses_, mask_, fov_x = load_blender(
        "train", scene=scene, path=nerf_path
    )
    # K_ = poser.get_K(H, W, fov_x * 180. / math.pi)
    K_ = poser.K
    input_image, input_K, input_pose, input_mask = (
        images_[index],
        K_,
        poses_[index],
        mask_[index],
    ...
    input_pose[:3, -1] = (
        input_pose[:3, -1] / np.linalg.norm(input_pose[:3, -1]) * poser.R
    )
    background_mask, image_mask = input_mask == 0.0, input_mask != 0.0
    input_image = cv2.resize(
        input_image, dsize=(256, 256), interpolation=cv2.INTER_CUBIC
    )
    image_mask = cv2.resize(
        image_mask.astype(np.float32), dsize=(256, 256), interpolation=cv2.INTER_NEAREST
    ).astype(bool)
    ...
    with tqdm(total=n_steps) as pbar, HeartBeat(pbar) as hbeat, EventStorage(
        folder_name
    ) as metric:
        ...
        with torch.no_grad():
            ...
            tforms = transforms.Compose(
                [transforms.Resize(256), transforms.CenterCrop((256, 256))]
            )
            ...
            # get input embedding
            model.clip_emb = (
                model.model.get_learned_conditioning(input_im.float())
                .tile(1, 1, 1)
                .detach()
            )
            model.vae_emb = (
                model.model.encode_first_stage(input_im.float()).mode().detach()
            )
        ...
        for i in range(n_steps):
            ...
            if train_view:
                ...
                with torch.enable_grad():
                    y_, depth_, ws_ = render_one_view(
                        vox, aabb, H, W, input_K, input_pose, return_w=True
                    )
                    y_ = model.decode(y_)
                ...
                input_smooth_loss = (
                    depth_smooth_loss(depth_) * depth_smooth_weight * 0.1
                )
                ...
                if train_view and i % 100 == 0:
                    metric.put_artifact(
                        "input_view",
                        ".png",
                        lambda fn: imwrite(fn, torch_samps_to_imgs(y_)[0]),
                    )

            # y: [1, 4, 64, 64] depth: [64, 64]  ws: [n, 4096]
            y, depth, ws = render_one_view(
                vox, aabb, H, W, Ks[i], poses[i], return_w=True
            )
            ...
            y_near, depth_near, ws_near = render_one_view(
                vox, aabb, H, W, Ks[i], near_pose, return_w=True
            )
            near_loss = (
                (y_near - y).abs().mean() + (depth_near - depth).abs().mean()
            ) * near_view_weight
            ...
            if isinstance(model, StableDiffusion):
                ...
                y = torch.nn.functional.interpolate(
                    y, (target_H, target_W), mode="bilinear"
                )

            with torch.no_grad():
                chosen_σs = np.random.choice(ts, bs, replace=False)
                chosen_σs = chosen_σs.reshape(-1, 1, 1, 1)
                chosen_σs = torch.as_tensor(
                    chosen_σs, device=model.device, dtype=torch.float32
                )
                # chosen_σs = us[i]
            ...
            emptiness_loss = (
                torch.log(1 + emptiness_scale * ws) * (-1 / 2 * ws)
            ).mean()  # negative emptiness loss
